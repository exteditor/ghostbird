...
[@exteditor/ghostbird](../modules.md) / CONTRIBUTING
...
* Install Yarn by `corepack install && corepack enable`.
  * See [`.devcontainer/init.sh`](../_media/init.sh) for example.
...
* You can adjust [`.editorconfig`](../_media/.editorconfig) and [`biome.json`](../_media/biome.json) to your taste locally, but please make sure to run `yarn fix` with the original config before sending a PR.
